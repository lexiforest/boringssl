...
void SSL_set_key_shares_limit(SSL *ssl, uint8_t limit) {
  ...
  ssl_impl->config->key_shares_limit = limit;
}

void SSL_CTX_set_key_shares_limit(SSL_CTX *ctx, uint8_t limit) {
  FromOpaque(ctx)->key_shares_limit = limit;
}
