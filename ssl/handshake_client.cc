...
static enum ssl_hs_wait_t do_send_client_key_exchange(SSL_HANDSHAKE *hs) {
  ...
  if (ssl_cipher_uses_certificate_auth(hs->new_cipher) &&
    ...
    // curl-impersonate: optionally disable the certificate key-usage check.
  ...
  if (alg_k & SSL_kRSA) {
    ...
    UniquePtr<BIGNUM> p(
        BN_bin2bn(hs->dh_p.data(), hs->dh_p.size(), nullptr));
    UniquePtr<BIGNUM> g(
        BN_bin2bn(hs->dh_g.data(), hs->dh_g.size(), nullptr));
    if (p == nullptr || g == nullptr) {
      return ssl_hs_error;
    }

    const unsigned bits = BN_num_bits(p.get());
    const size_t p_len = BN_num_bytes(p.get());
    if (bits < 1024) {
    ...
    DH_get0_key(dh.get(), &public_key, nullptr);
    if (public_key == nullptr ||
        !BN_bn2cbb_padded(&child, p_len, public_key)) {
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
      return ssl_hs_error;
    }
    ...
    if (peer_point == nullptr ||
        !pms.InitForOverwrite(DH_size(dh.get())) ||
        (secret_len = DH_compute_key(pms.data(), peer_point, dh.get())) <= 0) {
      BN_free(peer_point);
      ssl_send_alert(ssl, SSL3_AL_FATAL, SSL_AD_INTERNAL_ERROR);
      return ssl_hs_error;
    }
